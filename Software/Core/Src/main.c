...
uint32_t i2C_SemaphoreBuffer[4];
static osSemaphoreId_t i2C_SemaphoreHandle;
static osSemaphoreDef_t i2c_SemaphoreConfig ={ .name = "I2CLock", .cb_mem = 1, .cb_size = 1 };
...
int main(void)
{
  ...
  i2C_SemaphoreHandle = osSemaphoreNew( 1, 0, &i2c_SemaphoreConfig );
...
uint8_t AquireI2CPerpheral(void)
{
	if(i2C_SemaphoreHandle){
	  return osSemaphoreAcquire( i2C_SemaphoreHandle,5000);
	}
	return osOK;
}
void ReleaseI2CPeripheral(void)
{
  if(i2C_SemaphoreHandle)
  {
	  osSemaphoreRelease(i2C_SemaphoreHandle);
  }
